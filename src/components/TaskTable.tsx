...
import { DerivedTask, Task, Priority, Status, TaskInput } from '@/types';
...
export default function TaskTable({ tasks, onAdd, onUpdate, onDelete }: Props) {
  ...
  const handleEditClick = (task: Task, event: React.MouseEvent) => {
    // BUG FIX: Stop event propagation to prevent row click handler
                ...
                <TableRow key={t.id} hover onClick={() => setDetails(t)} sx={{ cursor: 'pointer' }}>
                  <TableCell>
                    <Stack spacing={0.5}>
                      ...
                      {t.notes && (
                        // BUG FIX: Use safe text rendering instead of dangerouslySetInnerHTML to prevent XSS
                        <Typography
                          variant="caption"
