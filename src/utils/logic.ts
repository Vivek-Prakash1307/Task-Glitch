...
export function computeROI(revenue: number, timeTaken: number): number | null {
  // BUG FIX: Validate inputs and handle edge cases safely
  if (!Number.isFinite(revenue) || !Number.isFinite(timeTaken) || timeTaken <= 0) {
    return null; // Return null for invalid inputs instead of Infinity/NaN
  }
...
export function sortTasks(tasks: ReadonlyArray<DerivedTask>): DerivedTask[] {
  return [...tasks].sort((a, b) => {
    ...
    // BUG FIX: Use deterministic tie-breaker (alphabetical by title) instead of random
