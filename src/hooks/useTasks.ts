...
export function useTasks(): UseTasksState {
  ...
  function normalizeTasks(input: any[]): Task[] {
    ...
    return (Array.isArray(input) ? input : []).map((t, idx) => {
      // BUG FIX: Add validation and safe defaults for all fields
      const id = typeof t.id === 'string' && t.id.trim() ? t.id.trim() : `task-${now}-${idx}`;
      const title = typeof t.title === 'string' && t.title.trim() ? t.title.trim() : `Untitled Task ${idx + 1}`;
      const revenue = Number.isFinite(Number(t.revenue)) && Number(t.revenue) >= 0 ? Number(t.revenue) : 0;
      const timeTaken = Number.isFinite(Number(t.timeTaken)) && Number(t.timeTaken) > 0 ? Number(t.timeTaken) : 1;
      const priority = ['High', 'Medium', 'Low'].includes(t.priority) ? t.priority : 'Medium';
      const status = ['Todo', 'In Progress', 'Done'].includes(t.status) ? t.status : 'Todo';
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        // BUG FIX: Removed malformed data injection that was adding invalid tasks
  ...
  // BUG FIX: Removed duplicate fetch effect that was causing double loading
